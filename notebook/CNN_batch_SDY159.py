...
#Pre-defined variables & shortcuts
Tensor = torch.Tensor
...
#Generalization 
CNN_from = "CHOOSEMODEL"
#config1 is for GPT, config2 is for LSTM, and config3 is for BiLSTM
config1 = {
    "model": {
        "batch_size": 64,
        "dropout": 0.0,
    ...
    "training": {
        ...
        "batch_size": 64,
        "training_h5_file": "Your Path or training hdf5 file",
        "block_size": 50,
        "ndata": -1,
        "num_epoch": 150, #100
        "learning_rate": 0.001,
        "scheduler_step_size": 250,
        'Job_number': "add number after learning curve file name"
    },
    "validating": {
        "validating_h5_file": "Your Path or validating hdf5 file",
        "batch_size": 16,
        "block_size": 50,
        "stride": 3000,
        "ndata": -1
    ...
    "testing": {
        "testing_h5_file": "Your Path or testing hdf5 file",
        "batch_size": 16,
        "block_size": 50,
        "stride": 3000,
        "ndata": -1
...
#Create Dataloader 
def createLoader(file_path: str, block_size: int, ndata: int = -1, batch_size: int = 64, shuffle: bool = False, data_type: str = None) -> DataLoader:
    logging.info(f'Creating {data_type} loader')
    ...
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=data_collator, drop_last=True)
    
    return loader, mu, std

training_loader, tr_mu, tr_std = createLoader(config["training"]["training_h5_file"], config["training"]["block_size"], config["training"]["ndata"], config["training"]["batch_size"], True, "training")
validating_loader, vl_mu, vl_std = createLoader(config["validating"]["validating_h5_file"], config["validating"]["block_size"], config["validating"]["ndata"], config["validating"]["batch_size"], False, "validating")
testing_loader, te_mu, te_std = createLoader(config["testing"]["testing_h5_file"], config["testing"]["block_size"], config["testing"]["ndata"], config["testing"]["batch_size"], False, "testing")
...
#CNN model
class CNN(nn.Module):
    def __init__(self, num_filters=[NUMFILTERS], kernel_size=KERNELSIZE, stride=STRIDE, activation_function=None, additional_activation_function=None):
        super(CNN, self).__init__()
        
        self.conv1 = nn.Conv3d(in_channels=1, out_channels=num_filters[0], kernel_size=kernel_size, stride=stride, padding=1)
        self.conv2 = nn.Conv3d(in_channels=num_filters[0], out_channels=num_filters[1], kernel_size=kernel_size, stride=stride, padding=1)
        self.conv3 = nn.Conv3d(in_channels=num_filters[1], out_channels=num_filters[2], kernel_size=kernel_size, stride=stride, padding=1)
        self.conv4 = nn.Conv3d(in_channels=num_filters[2], out_channels=num_filters[3], kernel_size=kernel_size, stride=stride, padding=1)
        self.maxpool = nn.MaxPool3d(kernel_size=MAXPOOLKERNEL, padding=1)
        self.flat = nn.Flatten()
        self.activation_function = activation_function
        self.additional_activation_function = additional_activation_function
        # Calculate the output size of the linear layer dynamically
        example_input = torch.randn(config["training"]["batch_size"], 1, depth, height, width)
        with torch.no_grad():
            conv_out = self.maxpool(self.conv4(self.maxpool(self.conv3(self.maxpool(self.conv2(self.maxpool(self.conv1(example_input))))))))
        flattened_size = conv_out.size(1) * conv_out.size(2) * conv_out.size(3) * conv_out.size(4) # Calculate the flattened size
        self.linear1 = nn.Linear(flattened_size, 38312)
        
    def forward(self, x):
        out = self.activation_function(self.maxpool(self.conv1(x)))
        out = self.activation_function(self.maxpool(self.conv2(out)))
        out = self.activation_function(self.maxpool(self.conv3(out)))
        out = self.activation_function(self.maxpool(self.conv4(out)))

        out = self.flat(out)
        out = self.linear1(out)
        out = self.additional_activation_function(out)
        return out
...
#Defining model
activation_function = AAAAA 
activation_function_at_linear = BBBBB
model = CNN(activation_function=activation_function, additional_activation_function=activation_function_at_linear)
...
torch.save(model.state_dict(), config["model"]["path"] + "/model/CNN_Pytorch.pt" )

#Calculate the trained model performance with testset
input1=[]
output=[]
params1=[]
...
print("MSE: %.5f" % MSE)
with open(config["model"]["path"] + '/metrics.txt', 'w') as f:
    f.write(f'R2: {R2:.8f}\n')
    f.write(f'MSE: {MSE:.8f}\n')
    f.write(f'Time: {((start.elapsed_time(end)) / 3600000):.8f}\n')
...
plt.title("kernel: KERNELSIZE, filters: [NUMFILTERS], stride: STRIDE, act_ftn_conv/pool: AAAAA, act_ftn_linear: BBBBB, kfold: KFOLD")
...
for k in range(0, y_origin.size(0), 10):
    df1 = pd.DataFrame(y_origin[k].detach().numpy())
    df_1 = df1 * df_2.to_numpy()[testing_index[k]]
    W = df_1[0].values
